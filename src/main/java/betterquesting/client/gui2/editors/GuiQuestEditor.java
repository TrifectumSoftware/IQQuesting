...
public class GuiQuestEditor extends GuiScreenCanvas implements IPEventListener, IVolatileScreen, INeedsRefresh {
    ...
    public void refreshGui() {
        ...
        if (quest == null) {
            ...
            pnTitle.setText(
                QuestTranslation
                    .translate("betterquesting.title.edit_quest", QuestTranslation.translateQuestName(questID, quest)));
    ...
    public boolean onMouseClick(int mx, int my, int button) {
        ...
        if (!quest.getProperty(NativeProps.NAME)
            .equals(flName.getValue())) {
            quest.setProperty(NativeProps.NAME, flName.getValue());
            flag = true;
        }

        if (!quest.getProperty(NativeProps.DESC)
            .equals(flDesc.getValue())) {
